...
export function initCityMap(city) {
      ...
      day.activities.forEach((activity, idx) => {
        ...
        if (!activity.isGeneric && mapsUrl) {
          popup += `<div class="popup-links">
            <a href="${mapsUrl}" target="_blank" rel="noopener" class="popup-link"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg> Ver en Maps</a>
            ${directionsUrl ? `<a href="${directionsUrl}" target="_blank" rel="noopener" class="popup-link directions"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg> Cómo llegar</a>` : ''}
          </div>`;
        }
  ...
  if (data.hotel?.coords) {
    ...
    const hotelDirectionsUrl = `https://www.google.com/maps/dir/?api=1&destination=${data.hotel.coords[0]},${data.hotel.coords[1]}&travelmode=transit`;
    
    let hotelPopup = `<h4>${data.hotel.name}</h4><p>Alojamiento</p>`;
    if (hotelMapsUrl) {
      hotelPopup += `<div class="popup-links">
        <a href="${hotelMapsUrl}" target="_blank" rel="noopener" class="popup-link"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg> Ver en Maps</a>
        <a href="${hotelDirectionsUrl}" target="_blank" rel="noopener" class="popup-link directions"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg> Cómo llegar</a>
      </div>`;
    }
...
function generateLegendByDay(data) {
  ...
  Object.keys(data.days).forEach((dateKey) => {
    ...
    day.activities.forEach((activity, idx) => {
      ...
      let actionsHtml = '';
      if (!activity.isGeneric && mapsUrl) {
        actionsHtml = `<div class="legend-actions">
          <a href="${mapsUrl}" target="_blank" rel="noopener" class="legend-action-btn" title="Ver en Google Maps"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></a>
          ${directionsUrl ? `<a href="${directionsUrl}" target="_blank" rel="noopener" class="legend-action-btn directions" title="Cómo llegar (Transporte Público)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg></a>` : ''}
        </div>`;
      }
  ...
  if (hotelInfo && data.hotel) {
    ...
    if (hotelMapsUrl) {
      actionsHtml = `<div class="legend-actions">
        <a href="${hotelMapsUrl}" target="_blank" rel="noopener" class="legend-action-btn" title="Ver en Google Maps"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></a>
        ${hotelDirectionsUrl ? `<a href="${hotelDirectionsUrl}" target="_blank" rel="noopener" class="legend-action-btn directions" title="Cómo llegar"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg></a>` : ''}
      </div>`;
    }
